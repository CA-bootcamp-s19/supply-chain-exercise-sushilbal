...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale(uint sku){
    ...
    require(items[sku].state == State.ForSale &&  items[sku].price > 0, 'Item is not ready for sale');
    _;
  }
  modifier sold(uint sku){
    
    require(items[sku].state == State.Sold,'Item is not sold yet.');
    _;
  }
  modifier shipped(uint sku){
    
    require(items[sku].state == State.Shipped,'Item is yet to be shipped' );
    _;
  }
